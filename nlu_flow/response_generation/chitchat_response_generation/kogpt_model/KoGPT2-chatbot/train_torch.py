...
class CharDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        q_toked = (
            ...
            + [self.eos,]
            + [self.sent_token]
            + self.tokenizer(sentiment)
            + [self.eos,]
        )
        q_len = len(q_toked)
        a_toked = (
            [self.a_token,] + self.tokenizer(a) + [self.eos,]
        )
        a_len = len(a_toked)
...
class KoGPT2Chat(LightningModule):
    ...
    def train_dataloader(self):
        ...
        self.train_set = CharDataset(
            data, self.tok_path, self.vocab, max_len=self.hparams.max_len
        )
    ...
    def inference(self, q: str, sent="0", max_token_len=32):
        ...
        with torch.no_grad():
            ...
            predicted_tokens = []
            prev_token_length = -1
            while len(predicted_tokens) <= max_token_len:
                input_ids = torch.LongTensor(
                    [self.vocab[U_TKN]]
                    + self.vocab[q_tok]
                    + self.vocab[EOS, sent]
                ...
                predicted_tokens = self.vocab.to_tokens(torch.argmax(pred, dim=-1).squeeze().numpy().tolist())
                gen = predicted_tokens[-1]
                if gen == EOS:
                    break
...
if __name__ == "__main__":
    if args.train:
        ...
        responses = meta_db_client.get('nlu-chitchat-responses')
        for response in responses:
            try:
                response_class = response['class_name']['classes']
                if response_class not in chitchat_response_dict.keys():
                    chitchat_response_dict[response_class] = []
                chitchat_response_dict[response_class].append(response['response'].replace(',',''))
            except:
                print(f'check response data: {response}')

        questions = meta_db_client.get('nlu-chitchat-utterances')

        with open('Chatbot_data/ChatbotData.csv', 'a') as data:
            data.write('\n')
            for question in questions:
                for answer in chitchat_response_dict[question['class_name']['classes']]:
                    data.write(question['utterance'].replace(',',''))
                    data.write(',')
                    data.write(answer.replace(',',''))
                    data.write(',')
                    data.write('0\n')
        ...
        trainer = Trainer.from_argparse_args(
            args, checkpoint_callback=checkpoint_callback, gradient_clip_val=1.0
        )
        trainer.fit(model)
