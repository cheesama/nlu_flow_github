...
class ChatbotKorpusDataset(torch.utils.data.Dataset):
    def __init__(self, questions, answers, tokenizer):
        ...
        for i, question in tqdm(enumerate(questions), desc="preparing data ..."):
            question_tokens = self.tokenizer.encode(questions[i], max_length=MAX_LEN, pad_to_max_length=True)
            answer_tokens = self.tokenizer.encode(answers[i], max_length=MAX_LEN, pad_to_max_length=True)
...
for question in tqdm(meta_questions, desc='meta db chitchat dataset adding ...'):
    ...
    answers.append(random.choice(chitchat_class_dict[question['class_name']['classes']][1]))
